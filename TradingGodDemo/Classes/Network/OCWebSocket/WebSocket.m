...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket
{
    ...
    if (webSocket == self.socket) {
        NSLog(@"************************** socket 连接成功************************** ");
        NSDictionary *dic = @{@"op":@"subscribe",@"args":@[@{@"channel":@"instruments",@"instType":@"FUTURES"}]};
        [[WebSocket shareWebSocketManage] sendMessage:dic chartPeriod:0 description:@"订阅请求啊啊啊"];
    }
}
