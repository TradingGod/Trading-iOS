...
class MainChartRenderer: BaseChartRenderer {
    ...
    override func drawChart(context: CGContext, lastPoint: TGKLineModel?, curPoint: TGKLineModel, curX: CGFloat) {
        if !isLine {
            drawCandle(context: context, curPoint: curPoint, curX: curX)
        }
        if let _lastPoint = lastPoint {
            if isLine {
                drawKLine(context: context, lastValue: _lastPoint.c, curValue: curPoint.c, curX: curX)
            } else if state == TGMainState.ma {
                drawMaLine(context: context, lastPoint: _lastPoint, curPoint: curPoint, curX: curX)
            } else if state == TGMainState.boll {
    ...
    func drawMaLine(context: CGContext, lastPoint: TGKLineModel, curPoint: TGKLineModel, curX: CGFloat) {
    ...
    func drawBOLL(context: CGContext, lastPoint: TGKLineModel, curPoint: TGKLineModel, curX: CGFloat) {
        if curPoint.up != 0 {
    ...
    func drawCandle(context: CGContext, curPoint: TGKLineModel, curX: CGFloat) {
        let high = getY(curPoint.h)
        let low = getY(curPoint.l)
        let open = getY(curPoint.o)
        let close = getY(curPoint.c)
    ...
    override func drawTopText(context _: CGContext, curPoint: TGKLineModel) {
