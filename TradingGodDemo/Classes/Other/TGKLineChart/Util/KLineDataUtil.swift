...
class KLineDataUtil {
    static func calculate(dataList: [TGKLineModel]) {
        let _dataList = dataList.reversed() as [TGKLineModel]
        calcMA(_dataList)
        calcBOLL(_dataList)
    ...
    // 增量更新时计算最后一个数据
    static func addLastData(dataList: [TGKLineModel], data: TGKLineModel) {
        var _dataList = dataList.reversed() as [TGKLineModel]
    ...
    // 更新最新的数据
    static func updateLastData(dataList: [TGKLineModel], data: TGKLineModel) -> [TGKLineModel] {
    ...
    static func calcMA(_ dataList: [TGKLineModel], isLast: Bool = false) {
        var ma5: CGFloat = 0
        var ma10: CGFloat = 0
        var ma20: CGFloat = 0
        var ma30: CGFloat = 0
    ...
    static func calcBOLL(_ dataList: [TGKLineModel], isLast: Bool = false) {
    ...
    static func calcMACD(_ dataList: [TGKLineModel], isLast: Bool = false) {
        var ema12: CGFloat = 0
        var ema26: CGFloat = 0
        var dif: CGFloat = 0
        var dea: CGFloat = 0
    ...
    static func calcVolumeMA(_ dataList: [TGKLineModel], isLast: Bool = false) {
    ...
    static func calcRSI(_ dataList: [TGKLineModel], isLast: Bool = false) {
        var rsi: CGFloat = 0
        ...
        for i in startIndex ..< dataList.count {
            ...
            if i == 0 {
                ...
                let Rmax = max(0, closePrice - dataList[i - 1].c)
                let RAbs = abs(closePrice - dataList[i - 1].c)
    ...
    static func calcKDJ(_ dataList: [TGKLineModel], isLast: Bool = false) {
        var k: CGFloat = 0
        var d: CGFloat = 0
...
//            }
            for index in startIndex ... i {
                max14 = max(max14, dataList[index].h)
                min14 = min(min14, dataList[index].l)
            }
    ...
    static func calcWR(_ dataList: [TGKLineModel], isLast: Bool = false) {
        var i_index = 0
...
//            }
            ...
            if i < 13 {
                ...
                if (max14 - min14) == 0 {
                    ...
                    entity.r = 100 * (max14 - dataList[i].c) / (max14 - min14)
                }
            }
        }
    }
